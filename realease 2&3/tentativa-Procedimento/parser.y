...
BLOCO:
	...
	| DECL_VAR MAIS_PROCEDIMENTOS { ESCOPO_VAR = 0; } BLOCO_MAIN 
	| BLOCO_MAIN
	| DECL_VAR BLOCO_MAIN
	| MAIS_FUNCOES { ESCOPO_VAR = 0; } BLOCO_MAIN ;
	| MAIS_PROCEDIMENTOS { ESCOPO_VAR = 0; } BLOCO_MAIN ;
...
MAIS_PROCEDIMENTOS:
	  MAIS_PROCEDIMENTOS DECL_PROCEDIMENTO token_proc_fim
	| DECL_PROCEDIMENTO token_proc_fim 
...
DECL_PROCEDIMENTO: 
	token_proc token_ident STRCPY_PROC PARAMETROS INSERE_PROC_TAB token_pt_virg BLOCO_PROC
	| token_proc token_ident STRCPY_PROC PARAMETROS INSERE_PROC_TAB token_pt_virg ;	


STRCPY_FUNC: { strcpy(nome_func, ident); strcpy(nome_func2, nome_func); ESCOPO_VAR++;} ;
STRCPY_PROC: { strcpy(nome_proc, ident); strcpy(nome_proc2, nome_proc); ESCOPO_VAR++;} ;
...
INSERE_PROC_TAB: {
	func = busca(tabela_hash_func, nome_proc, ESCOPO_VAR);
	if(func) {
		printf(" Erro semantico: Procedimento %s redeclarado. Escopo: %d, Linha: %d \n", nome_proc, ESCOPO_VAR, nlinhas);
		exit(0);
	}
	tabela_hash_func = insere_funcao(tabela_hash_func, nome_proc, TIPO_VAR, qtd_param);
	tipo_parametros[qtd_param] = -1;
	tabela_hash_func = insere_parametro_funcao(tabela_hash_func, nome_proc, tipo_parametros);
	qtd_param = 0;
};
...
CHAMADA_FUNCAO: 
	...
	{	
		func = busca(tabela_hash_func, nome_proc, ESCOPO_VAR);
		if (func == NULL){
				printf(" Erro semantico: Procedimento nao declarada. Procedimento: %s, Linha: %d.\n", nome_proc, nlinhas);
				exit(0);
		}
		if (get_aridade(func) != qtd_param){
				printf(" Erro semantico: Quantidade de parametros errada. Procedimento %s, Linha: %d.\n", nome_proc, nlinhas);
				exit(0);
		}
		if (verifica_tipo_parametros_funcao(tabela_hash_func, nome_proc, doge) == 0){
				printf(" Erro semantico: Tipo de parametros errado. Procedimento: %s, Linha: %d.\n", nome_proc, nlinhas);			
				exit(0);
		}
		qtd_param = 0;
		doge = inicializa();	
	...
	| STRCPY_IDENT token_abreParen token_fechaParen { 
		strcpy(nome_proc, ident);
		func = busca(tabela_hash_func, nome_proc, ESCOPO_VAR);
		if (func == NULL){
				printf(" Erro semantico: Procedimento nao declarado. Procidmento: %s, Linha: %d.\n", nome_proc, nlinhas);
				exit(0);
		}
		if (get_aridade(func) > 0){
				printf(" Erro semantico: Quantidade de parametros errada. Procidmento: %s, Linha: %d.\n", nome_proc, nlinhas);
				exit(0);
		}
	};
